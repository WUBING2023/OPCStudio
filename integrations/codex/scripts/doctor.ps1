...
$checks = [ordered]@{}
$checks.hostCommand = [bool](Get-Command codex -ErrorAction SilentlyContinue)

$checks.hostManifest = Test-Path (Join-Path $PluginRoot ".codex-plugin\plugin.json")
...
try {
  $hostManifest = Get-Content -Raw (Join-Path $PluginRoot ".codex-plugin\plugin.json") | ConvertFrom-Json
...
if (-not $SkipHostRegistration -and $checks.hostCommand) {
  $registration = (& codex plugin marketplace list 2>&1 | Out-String)
  $checks.hostRegistration = $registration -match [regex]::Escape("opc-studio-codex")
} else {
...
[pscustomobject]@{ ok = [bool]$ok; platform = "codex"; setupState = $setupState; setupReason = $setupReason; checks = $checks } | ConvertTo-Json -Depth 6
if (-not $ok) { exit 1 }
